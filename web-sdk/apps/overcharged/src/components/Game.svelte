...
<App>
	...
	{:else}
		...
		<Win />
		<FreeSpinIntro />
		{#if ['desktop', 'landscape', 'portrait'].includes(context.stateLayoutDerived.layoutType())}
			<FreeSpinCounter />
		{/if}
		<FreeSpinOutro />
		<!--
			Skill aktivasyon görseli artık tamamen skillActivated spine'ı
			(SkillActivatedOverlay). Eski programatik VFX (SkillVFX: radial burst,
			bolt rain vb.) kaldırıldı — yalnızca spine animasyonu oynuyor.
		-->
